...
# 2018-11-13
# 150 scans before cleaning, 18 after
...
metadata <- gs_url('https://docs.google.com/spreadsheets/d/1QXxbphyus5Ql8kqWqY1uGTmebdSS9QkRWDQgZIVUnLc/edit#gid=0')
meta <- gs_read_csv(metadata) %>% 
  arrange(prefix, spectrum_number)
  meta

# add groups and spec_num
meta <- meta %>% 
  mutate(group = rep(1:56, each = 3),
         spec_num = c(rep(1:3, times = 56) ) )
...
working_folder <- '2018-11-12-Calibration SPC_99AA02-0318-9711'
...
all_wvls <- filter(spec_all, filename == 'gr111218_0000.sig')$wvl
...
# download the second calibration
tar_calib2 <- read.table('/Users/etienne/Documents/ProjetsRecherche/CABO/panel-calibrations/panels/99AA02-0318-9711/2018-07-19/SRS-99-010-99AA02-0318-9711-2018-07-19.calib') %>% 
  as_data_frame() %>% 
  rename(wvl = V1, refl = V2)
tar_calib2
...
spec_three <- spec_interpol_long %>% 
  filter(type == 'smooth') %>% 
  select(-type) %>% 
  bind_rows(tar_calib, tar_calib2) %>% 
  mutate(Calibration = calib,
         Calibration = replace(Calibration, Calibration == 'clean', '2018-11-13 (cleaned)'),
         Calibration = replace(Calibration, Calibration == 'dirty', '2018-11-12 (dirty)'))
...
spec_all_plot <- ggplot(spec_three, aes(x = wvl, y = refl)) +
  ...
  ylab('Reflectance')
spec_all_plot
ggsave('99AA02-0318-9711_spec_all_plot.png', plot = spec_all_plot, width = 6, height = 3, dpi = 600)
...
drive_upload('99AA02-0318-9711_spec_all_plot.png', path = path_name)
