...
calibration_date <- "2019-08-12" ## this is the date post-cleaning measurements where made
precalibration_date <- "2019-07-31" # this is the date pre-cleaning measurements where made
panel <- "9711"
prefix_wd <- paste0("./panels/", panel_id, "/",calibration_date)

prefix_before <- paste0(precalibration_date,"_before_cleaning/gr073119") 
prefix_after <- paste0(calibration_date,"_after_cleaning/gr081219")
...
meta_aft<- read_csv(paste0("./panels/order_measurements/order_measurements_2.csv"))
...
table(meta_sub$filename %in% spec_data_all_df$filename) #how many of the filnames in meta_sub are also in 

#since 2019-08-12 in spec all has two unutilized spectras, delete them before leftjoin 
#if not: rror: Column `cleaning` must be length 143360 (the number of rows) or one, not 145408
#delete spectras 10-11 from sepc_data_all_df
bad_spec<-paste0(prefix_after, "_", "00", c(10,11), rep(".sig",2))
bad_spec
spec_data_all_df<-spec_data_all_df %>% 
  filter(!filename %in% bad_spec)
...
# Take average dirty of all reps
...
spec_three <- spec_interpol_long %>%  ##here: operation needed: uptade clean and dirty date
  ...
  mutate(Calibration = calib,
         Calibration = replace(Calibration, Calibration == 'clean', '2019-08-12 (cleaned)'),
         Calibration = replace(Calibration, Calibration == 'dirty', '2019-07-31 (dirty)'))
