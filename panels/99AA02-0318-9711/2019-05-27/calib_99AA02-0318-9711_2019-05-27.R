...
prefix_before <- paste0(precalibration_date,"_before_cleaning/gr052319") 
prefix_after <- paste0(calibration_date,"_after_cleaning/gr052719")
...
spec_three <- spec_interpol_long %>% 
  ...
  select(-type) %>% 
  bind_rows(tar_calib, tar_calib2, tar_calib3, tar_calib4) %>% 
  mutate(Calibration = calib,
         Calibration = replace(Calibration, Calibration == 'clean', '2019-05-27 (cleaned)'),
         Calibration = replace(Calibration, Calibration == 'dirty', '2019-05-23 (dirty)'))

# Plot it toutes les anciennes calibrations + sal/propre de la calibration présente
