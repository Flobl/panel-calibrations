...
prefix_before <- paste0(precalibration_date,"_before_cleaning/gr062119") ## gr+ date per-cleaning
prefix_after <- paste0(calibration_date,"_after_cleaning/gr062519") ## gr+ date post-cleaning 
...
spec_three <- spec_interpol_long %>% 
  ...
  mutate(Calibration = calib,
         Calibration = replace(Calibration, Calibration == 'clean', '2019-06-25 (cleaned)'),
         Calibration = replace(Calibration, Calibration == 'dirty', '2019-06-25 (dirty)'))
