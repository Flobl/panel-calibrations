...
calibration_date <- "2019-09-19" ## this is the date post-cleaning measurements where made
precalibration_date <- "2019-09-17" # this is the date pre-cleaning measurements where made
panel <- "9711"
prefix_wd <- paste0("./panels/", panel_id, "/",calibration_date)

prefix_before <- paste0(precalibration_date,"_before_cleaning/gr091719") 
prefix_after <- paste0(calibration_date,"_after_cleaning/gr091919")
...
tar_calib6 <- read.table(tar_ls[6]) %>% 
  as_tibble() %>% 
  rename(wvl = V1, refl = V2)
...
spec_three <- spec_interpol_long %>%  ##here: operation needed: uptade clean and dirty date
  ...
  mutate(Calibration = calib,
         Calibration = replace(Calibration, Calibration == 'clean', '2019-09-19 (cleaned)'),
         Calibration = replace(Calibration, Calibration == 'dirty', '2019-09-17 (dirty)'))
