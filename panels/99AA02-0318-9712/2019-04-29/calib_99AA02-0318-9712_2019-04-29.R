...
prefix_before <- paste0(precalibration_date,"_before_cleaning/gr042319") ## gr+ date per-cleaning
prefix_after <- paste0(calibration_date,"_after_cleaning/gr042919") ## gr+ date post-cleaning 
...
spec_three <- spec_interpol_long %>% 
  ...
  mutate(Calibration = calib,
         Calibration = replace(Calibration, Calibration == 'clean', '2019-04-29 (cleaned)'),
         Calibration = replace(Calibration, Calibration == 'dirty', '2019-04-23 (dirty)'))
