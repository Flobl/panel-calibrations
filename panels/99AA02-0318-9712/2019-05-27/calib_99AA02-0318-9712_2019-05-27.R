...
prefix_before <- paste0(precalibration_date,"_before_cleaning/gr052319") ## gr+ date per-cleaning
prefix_after <- paste0(calibration_date,"_after_cleaning/gr052719") ## gr+ date post-cleaning 
...
all(diff(test_wvls) >= 0) ### we want this to be TRUE
...
spec_three <- spec_interpol_long %>% 
  ...
  select(-type) %>% 
  bind_rows(tar_calib, tar_calib2, tar_calib3) %>% 
